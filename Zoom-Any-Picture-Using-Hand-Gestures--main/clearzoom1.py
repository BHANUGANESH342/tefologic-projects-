...
while True:
    ...
    img1 = cv2.resize(img1, (img.shape[1], img.shape[0]))  # Match the vidqeo feed dimensions
    ...
    if len(hands) == 2:
        ...
        # Define the region to replace with the resized image
        roi = img[0: img.shape[0], 0: img.shape[1]]

        # Ensure both images have the same channels
        img1_resized = img1_resized[:roi.shape[0], :roi.shape[1]]

        # Blend the resized image with the original image using the mask
        img[0:img.shape[0], 0:img.shape[1]] = img1_resized
